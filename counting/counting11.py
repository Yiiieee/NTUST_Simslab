...
CAMERA_SOURCE = 2
# CAMERA_SOURCE = 1
# CAMERA_SOURCE = "rtsp://username:password@ip-address/stream"
...
# Jangan hardcode service role key di file.
...
# ===================== 2 BOX LOGIC =====================
# MAIN_BOX = (250, 350, 550, 650)
# HUMAN_BOX = (250, 100, 550, 350)

MAIN_BOX = (550, 450, 1200, 650)
HUMAN_BOX = (550, 100, 1200, 450)
...
def cleanup_old_tracker_mappings(
    tracker_to_global,
    tracker_last_seen,
    current_frame_idx,
    ttl_frames=60
...
def apply_stock_change(supa, active_box_id, delta, event_name, last_update_time):
    ...
    if now - last_update_time < MIN_UPDATE_INTERVAL:
        return last_update_time, False, f"{event_name} skipped by throttle"
...
# ===================== DIRECT SAME-GID TRANSFER =====================
def resolve_direct_same_gid_zone_transition(
    gid,
    state,
    prev_zone,
    stable_zone,
    ...
    if not stable_changed:
        return last_update_time, 0, 0, None

    if prev_zone not in ("main", "human") or stable_zone not in ("main", "human"):
        return last_update_time, 0, 0, None

    if prev_zone == stable_zone:
        return last_update_time, 0, 0, None

    if prev_zone == "main" and stable_zone == "human" and not state.get("taken", False):
        last_update_time, ok, msg = apply_stock_change( 
            supa=supa,
            active_box_id=active_box_id,
            delta=-1,
            event_name=f"DIRECT SAME GID MAIN->HUMAN (gid {gid})",
        ...
        if ok:
            ...
            return last_update_time, 1, 0, msg

        return last_update_time, 0, 0, msg

    if prev_zone == "human" and stable_zone == "main" and state.get("taken", False):
        last_update_time, ok, msg = apply_stock_change(
            supa=supa,
            active_box_id=active_box_id,
            delta=+1,
            event_name=f"DIRECT SAME GID HUMAN->MAIN (gid {gid})",
            last_update_time=last_update_time,
        )
        ...
        if ok:
            ...
            close_transfer_candidate(state)
            return last_update_time, 0, 1, msg
...
def resolve_transfer_candidates(
    ...
    for candidate_gid in candidate_gids:
        ...
        for target_gid in target_gids:
            ...
            if from_zone == "main" and (
                candidate_state.get("taken", False)
                or target_state.get("taken", False)
            ):
                continue
        ...
        if (
            from_zone == "main"
            and target_state["last_stable_zone"] == "human"
            and not candidate_state["taken"]
            and not target_state["taken"]
        ):
            last_update_time, ok, msg = apply_stock_change(
                supa=supa,
                active_box_id=active_box_id,
            ...
            from_zone == "human"
            and target_state["last_stable_zone"] == "main"
            and candidate_state["taken"]
        ):
            last_update_time, ok, msg = apply_stock_change(
                supa=supa,
                active_box_id=active_box_id,
...
def main():
    ...
    if not cap.isOpened():
        write_state(last_event=f"Cannot open camera: {CAMERA_SOURCE}")
    ...
    print(f"[INFO] Camera source: {CAMERA_SOURCE}")
    ...
    direct_same_gid_transfer_count = 0

    write_state(active_box_id=None, last_event="CV live camera running...", last_decrement=None)
    ...
    while True:
        ...
        if not ret:
            write_state(
                active_box_id=active_box_id,
                last_event="Camera frame read failed.",
                last_decrement=None
            )
            ...
            if result.boxes is not None and result.boxes.id is not None:
                ...
                for (x, y, w, h), conf, tid in zip(boxes, confs, track_ids):
                    ...
                    if gid not in global_states:
                        ...
                        global_states[gid] = init_global_state(
                            frame_idx,
                            initial_zone=init_zone
                        )
                        new_global_count += 1
                    ...
                    last_update_time, direct_take_delta, direct_return_delta, direct_msg = resolve_direct_same_gid_zone_transition(
                        gid=gid,
                        state=state,
                        prev_zone=prev_zone,
                    ...
                    if direct_take_delta > 0 or direct_return_delta > 0:
                        take_count += direct_take_delta
                        return_count += direct_return_delta
                        direct_same_gid_transfer_count += direct_take_delta + direct_return_delta

                        if direct_msg:
                            print(direct_msg)

                    if (
                        state.get("transfer_candidate", False)
                        and stable_zone == state.get("transfer_from_zone")
                    ):
                        close_transfer_candidate(state)
        ...
        # ===================== HANDLE MISSING =====================
        for gid, state in list(global_states.items()):
            if gid in active_global_ids_this_frame:
                continue
        ...
        cv2.putText(frame, f"Direct same GID: {direct_same_gid_transfer_count}", (40, 270),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, f"Ghost same GID: {same_gid_transfer_count}", (40, 300),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, f"Cross GID transfer: {cross_gid_transfer_count}", (40, 330),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, f"Global alive: {len(global_states)}", (40, 360),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, f"Ghost radius: {GHOST_MATCH_RADIUS}px", (40, 390),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, f"Taken sync radius: {TAKEN_SYNC_RADIUS}px", (40, 420),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, f"Transfer window: {TRANSFER_WINDOW_SECONDS:.1f}s", (40, 450),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.68, (255, 255, 255), 2)

        cv2.putText(frame, "Press Q to quit", (40, height - 30),
        ...
        if SHOW_MAIN_WINDOW:
            ...
            cv2.imshow("CV Counting Live Camera", frame)
        ...
        if key == ord("q"):
            write_state(
                active_box_id=active_box_id,
                last_event="CV live camera stopped by user.",
                last_decrement=None
            )
            break
