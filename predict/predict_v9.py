...
def draw_center_alert(frame, text, bg_color=(0, 0, 255), text_color=(255, 255, 255),
                      font_scale=1.3, thickness=3, pad_x=30, pad_y=22):
    """
    Draw alert text in center with solid background.
    """
    h, w, _ = frame.shape
...
def draw_center_alert_transparent(frame, text, bg_color=(0, 0, 255), text_color=(255, 255, 255),
                                  font_scale=1.3, thickness=3, pad_x=30, pad_y=22, alpha=0.75):
    """
    Draw alert text in center with semi-transparent background.
    """
    h, w, _ = frame.shape
    font = cv2.FONT_HERSHEY_SIMPLEX
...
def predict_live_camera(camera_index, model, transform, class_names, clip_len=16, device="cpu"):
    ...
    if save_video:
        ...
        out_path = "output_result_envproof3_alert_center.avi"
    ...
    # ALERT STATE
    # =====================================================
    show_assemble2_alert = False
    assemble2_started = False

    # optional: anti-flicker basic for Assemble2 start
    assemble2_detect_count = 0
    assemble2_detect_needed = 2  # increase if still too sensitive
    ...
    while True:
        ...
        if len(frame_buffer) == clip_len:
            ...
            # ==============================================
            # ALERT LOGIC
            # Alert muncul setelah Assemble1 selesai
            # Alert hilang saat Assemble2 mulai
            # ==============================================
            if class_states.get("Assemble1", {}).get("is_ok", False) and not assemble2_started:
                show_assemble2_alert = True

            if last_pred_class == "Assemble2" and last_conf >= conf_threshold:
                assemble2_detect_count += 1
            else:
                assemble2_detect_count = 0

            if assemble2_detect_count >= assemble2_detect_needed:
                assemble2_started = True
                show_assemble2_alert = False
        ...
        cv2.putText(frame, f"Pred: {last_pred_class} ({last_conf:.2f})", (10, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2, cv2.LINE_AA)

        # ==============================================
        # CENTER ALERT OVERLAY
        # ==============================================
        # Blink biar tidak bisa diabaikan
        if show_assemble2_alert and int(time.time() * 2) % 2 == 0:
